...
struct Config {
    ...
    url: String,
    /// The far node's local nickname. It becomes a family prefix, so it must
    /// be a verb segment.
    node: String,
}
...
fn main() -> std::io::Result<()> {
    ...
    Verb::new(&format!("{node}_family"), "verb").map_err(|_| {
        std::io::Error::other(format!(
            "node must be a verb segment ([a-z][a-z0-9_]*, no `__`): {node:?}"
        ))
    })?;

    let link = Arc::new(Link::new(&url));
    // Leaked because the mirroring thread outlives this frame and carries
    // the registrar, which borrows the plugin: one name per process, freed
    // when the process is.
    let name: &'static str = Box::leak(format!("portos-remote-{node}").into_boxed_str());
    let event_link = link.clone();
    let event_node = node.clone();

    // Serving comes first and discovery second, which is the opposite of
    // what this did. Asking the far node what it offers means being able to
    // reach it, and a node that is not up yet is not a reason to fail to
    // start — it is a reason to answer nothing until it is. The verbs are
    // taken on once the answer arrives; each carries its own peer name in
    // its own closure, so no table sits beside the dispatch to get out of
    // step with it.
    portos_sdk::serve(
        Plugin::new(name).on_ready(move |registrar, client| {
            // On its own thread, and retrying: a peer that is not up yet is
            // not a reason to fail to start. Until it answers, this driver
            // runs and answers nothing — the same state as a plugin waiting
            // on a dependency, because that is what it is.
        ...
        match discover(link, node) {
            Ok(mirrored) => {
                ...
                for m in mirrored {
                    ...
                    if let Err(e) =
                        registrar.tool(&m.local, &m.meta.description, schema, move |args, _c| {
                            link.invoke(&peer, args)
                                .map_err(|e| CallError::from(e.to_string()))
                        })
                    {
                        eprintln!("[remote] could not take on {}: {e}", m.local);
                    }
                }
                return;
            }
