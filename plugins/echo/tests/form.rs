...
///
/// `artifact` names one executable, which is all a Rust driver is and not
/// nearly what a JS one is — a script plus whatever it imports. A bundle is
/// the rest of the answer: a tar in the CAS that becomes the process's
/// working directory, so paths *inside* the plugin keep meaning what they
/// meant when it was built. The two are separate fields because they answer
/// separate questions: what runs, and what files it has. A JS plugin needs
/// one of each, and they come from different places — the script from the
/// bundle, `node` from the host.
#[test]
fn a_plugin_of_several_files_travels_as_one_bundle() {
    let (kernel, host, root) = setup("bundle");

    // A tar holding the plugin and nothing else, the way `portos bundle`
    // builds one.
    let tar_path = root.join("b.tar");
    {
        let mut builder = tar::Builder::new(std::fs::File::create(&tar_path).unwrap());
        builder
            .append_path_with_name(ECHO_BIN, "bin/portos-echo")
            .unwrap();
    ...
    let plugin = host
        .spawn_spec(&LaunchSpec {
            // A path *inside* the bundle, and relative on purpose: resolving
            // it is the kernel's job, not something left to whatever a
            // relative program name means after a chdir.
            env: [(
                "PORTOS_ECHO_GRANDCHILD".to_string(),
        ...
        .expect("a bundled plugin starts");
    ...
    let made: serde_json::Value = host
        ...
        .parse()
        .unwrap();
    assert!(made["ref"].is_string(), "and answers: {made}");

    // The relative filename the plugin was given landed inside the bundle,
    // which is the observable form of "the bundle is the working directory".
    let unpacked = root
        .join("exec")
        .join(format!("{}.d", meta.id.replace(':', "-")));
    assert!(
        unpacked.join("grandchild.pid").exists(),
        "a relative path inside a plugin means a path inside its bundle"
    );
    assert!(unpacked.join("bin/portos-echo").exists());

    // Unpacked once: the id is the bytes, so there is nothing to invalidate.
    let stamp = std::fs::metadata(unpacked.join("bin/portos-echo"))
        .unwrap()
        .modified()
        .unwrap();
    let second = host
        ...
        .unwrap();
    assert_eq!(
        std::fs::metadata(unpacked.join("bin/portos-echo"))
            .unwrap()
            .modified()
            .unwrap(),
        stamp,
        "a second spawn reuses the unpacked tree"
    );
...
fn exercise(host: &Host, form: Form) -> serde_json::Value {
    ...
    let call = |verb: &str, args: serde_json::Value| -> serde_json::Value {
        host.call(
            &plugin,
            &portos_abi::ids::Verb::parse(verb).unwrap(),
            portos_abi::wire::Payload::of(&args).unwrap(),
        )
        .unwrap()
        .parse()
        .unwrap()
