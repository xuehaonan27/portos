...
fn link_ready() -> Option<(PathBuf, PathBuf)> {
    ...
    Some((repo_root().join("plugins/bridge-http/bridge.js"), remote))
}
...
/// Wait for a verb to start being answered.
///
/// A remote driver has to ask the far node what it offers, and asking takes
/// as long as it takes — so `spawn` returning means "it is running", not "it
/// is answering". Any caller faces this; the tests face it too rather than
