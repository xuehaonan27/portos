...
fn tp(s: &str) -> portos_abi::ids::Topic {
    portos_abi::ids::Topic::parse(s).expect("test topic")
}
...
fn drain_turn(
    rx: &std::sync::mpsc::Receiver<portos_abi::wire::LocalEvent>,
...
fn agentic_loop_end_to_end_with_tool_call() {
    ...
    assert!(
        reqs[1].contains("e1"),
        "echo's ref value reached the provider"
    );
...
fn introspected_tools_and_artifact_read() {
    ...
    let meta = kernel
        ...
        .put_bytes(
            content.as_bytes(),
            "text/plain",
            portos_abi::Label::public_trusted(),
            "test",
        )
        .unwrap();
    ...
    assert!(
        reqs[0].contains("Print a line"),
        "driver-advertised description flowed through"
    );
    assert!(
        reqs[0].contains("artifact__read"),
        "built-in reader present"
    );
    assert!(
        !reqs[0].contains("egress__"),
        "egress must not surface as a model tool"
    );
