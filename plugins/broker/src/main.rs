...
fn load_config() -> Config {
    let Some(dir) = std::env::var_os(portos_egress_api::DIR_ENV) else {
        eprintln!(
            "[broker] {} unset: empty allowlist, denying all egress",
            portos_egress_api::DIR_ENV
        );
        return Config {
            rules: Vec::new(),
...
fn main() -> std::io::Result<()> {
    ...
    let (cfg_http, agent_http) = (cfg.clone(), agent.clone());
    let (cfg_stream, agent_stream) = (cfg.clone(), agent.clone());
...
/// caller-named topic as chunk events, closed by done (or error). The caller
