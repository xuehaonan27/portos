...
//!
//! **It is a filesystem with one rule: the name is derived from the
//! content.** Layout: `<root>/objects/<hh>/<rest-of-hash>`, one file per
//! artifact, holding exactly the bytes — `file(1)` recognises them, and so
//! does every other tool. That is deliberate and worth not losing: the store
//! is not a layer over a filesystem, it is a naming discipline on one.
//!
//! The rule earns its keep in four places a plain path cannot reach: the
//! same bytes get the same name on every machine (which is what makes a
//! plugin artifact portable), a handle still means the same bytes after five
//! intervening tool calls (a path can be rewritten underneath the model), what
//! ran is checkable afterwards, and a materialised copy never needs
//! invalidating. Content addressing is the cheapest possible enforcement of
//! write-once, and write-once is what the control/data plane split rests on.
//!
//! Objects are stored **read-only (0444)**, so that immutability is enforced
//! by the filesystem rather than promised by convention — which is what makes
//! it safe to hand a plugin the path of one instead of copying the bytes.
//!
//! Ingest is tmp + rename; reads stream out as chunked bytes on the data
//! plane — payloads never move through any JSON frame or model context.
...
use portos_abi::{ArtifactId, ArtifactMeta, Label, artifact::id_for_bytes};
...
impl Cas {
    ...
    pub fn put_stream<R: Read>(
        ...
        if path.exists() {
            ...
            std::fs::create_dir_all(path.parent().unwrap())?;
            // Read-only before it is visible under its final name: an object
            // is immutable, and a rule the filesystem enforces is worth more
            // than one every writer has to remember. It is also what makes
            // handing out the path safe.
    ...
    /// Where an artifact's bytes are, for a reader that would rather open
    /// the file than stream it through a socket.
    ///
    /// This is the half of the store that was missing: it has always been an
    /// ordinary file, and nothing but the ABI could reach it, so every use of
    /// an artifact had to go through the model's context — which is the one
    /// thing the store exists to prevent.
    pub fn path_of(&self, id: &ArtifactId) -> Result<PathBuf, KernelError> {
        let path = self.path_for(id)?;
        if path.exists() {
            Ok(path)
        } else {
            Err(KernelError::NotFound(format!("artifact: {id}")))
        }
    }
