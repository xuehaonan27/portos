...
impl CapStore {
    ...
    /// Whether `subject` could call this verb — without spending a budget.
    ///
    /// A separate method from [`find_and_exercise`] and not a flag on it: a
    /// check that sometimes consumes and sometimes does not is a check
    /// nobody can call safely. Readiness asks this one, repeatedly, and must
    /// not spend anything by asking.
    ///
    /// [`find_and_exercise`]: CapStore::find_and_exercise
    pub fn allows(&self, subject: &str, resource: &str, verb: &str, now: u64) -> bool {
        self.list_live(subject, now)
            .map(|caps| {
                caps.iter()
                    .any(|c| c.resource == resource && c.verbs.contains(verb))
            })
            .unwrap_or(false)
    }
