...
fn workspace_entry_passes_all_frozen_gates() {
    ...
    let vm = l
        .grant(
            ...
            GrantRequest {
                owner: SubjectId::new("agent"),
                claim: Claim::new(Ex::Token).unwrap(),
                generation: Generation::new("fc@1"),
                parent: Some(ws).map(|id| l.holding(id).expect("parent exists").handle()),
                lease: LeaseRequest::UseClassDefault,
                now: Timestamp::try_from(0u64).unwrap(),
            },
        )
    ...
    for (c, i) in [
        ...
        l.grant(
            &l.pool::<Ex>(&ResourceKey::new(ClassId::new(c), InstanceId::new(i)))
                .unwrap(),
            GrantRequest {
                owner: SubjectId::new("agent"),
                claim: Claim::new(Ex::Token).unwrap(),
                generation: Generation::new("g"),
                parent: Some(vm).map(|id| l.holding(id).expect("parent exists").handle()),
                lease: LeaseRequest::UseClassDefault,
                now: Timestamp::try_from(0u64).unwrap(),
            },
        )
    ...
    l.grant(
        &l.pool::<portos_rm::ra::Count>(&ResourceKey::new(
            ClassId::new("vcpu"),
            InstanceId::new("host"),
        ))
        .unwrap(),
        GrantRequest {
            owner: SubjectId::new("agent"),
            claim: Claim::new(portos_rm::ra::Count::Value(2)).unwrap(),
            generation: Generation::new("g"),
            parent: Some(vm).map(|id| l.holding(id).expect("parent exists").handle()),
            lease: LeaseRequest::UseClassDefault,
            now: Timestamp::try_from(0u64).unwrap(),
    ...
    .unwrap();
    l.grant(
        &l.pool::<portos_rm::ra::GSet>(&ResourceKey::new(
            ClassId::new("image"),
        ...
        GrantRequest {
            owner: SubjectId::new("agent"),
            claim: Claim::new(portos_rm::ra::GSet::of(&["ro"])).unwrap(),
            generation: Generation::new("g"),
            parent: Some(vm).map(|id| l.holding(id).expect("parent exists").handle()),
            lease: LeaseRequest::UseClassDefault,
            now: Timestamp::try_from(0u64).unwrap(),
    ...
    assert!(
        matches!(o.teardown("agent", 3, None), RunOutcome::Completed { failed } if failed.is_empty())
    );
    assert_eq!(o.ledger.live_snapshot(&SubjectId::new("agent")).len(), 0);
    assert_eq!(
        o.ledger.live_snapshot(&SubjectId::new("other")).len(),
        1,
        "别人的镜像份额不受影响"
    );
...
fn workspace_segment_rollback_restores_touched_vm_exactly_once() {
    ...
    assert_eq!(
        m.orch.world.restored().len(),
        1,
        "触及同一持有三次，只 restore 一次"
    );
    ...
    assert_eq!(
        m.trace
            .iter()
            .filter(|ev| matches!(ev, Ev::Restored { .. }))
            .count(),
        1
    );
    m.rollback_segment(10);
    assert_eq!(
        m.orch.world.restored().len(),
        1,
        "重复回滚不重复 restore（钥匙承重）"
    );
    assert_eq!(
        m.world.emitted.len(),
        3,
        "发射记录不因回滚消失——回滚的是持有状态，不是历史"
    );
}
...
fn workspace_weighted_budget_static_bound_covers_metered_spend() {
    ...
    let plan = Plan::Seq(vec![
        Plan::loop_(3, Plan::verb("vm", "exec")),
        Plan::loop_(2, Plan::verb("vm", "write_file")),
    ]);
    let d = admit_plan(
        &plan,
        &lookup,
        &ceiling(&e.mount.offers, &e.mount.offers),
        &e.mount.provides,
        &Budget::of(&[("vm::exec", 15), ("vm::write_file", 4)]),
    )
    .unwrap();
    assert_eq!(
        d.uses,
    ...
    let mut m = Monitor::new(pol, Mode::Strict, Ledger::new(), "fib");
    let metered = vec![
        WAction::new("exec", "vm-1", "a", 3),
        WAction::new("exec", "vm-1", "b", 4),
        WAction::new("exec", "vm-1", "c", 5),
        WAction::new("exec", "vm-1", "d", 6),
    ];
...
fn rdma_entry_passes_all_frozen_gates_with_interval_and_frac() {
    ...
    let dev = l
        .grant(
            ...
            GrantRequest {
                ...
                claim: Claim::new(portos_rm::ra::GSet::of(&["open"])).unwrap(),
                generation: Generation::new("g"),
                parent: None,
                lease: LeaseRequest::UseClassDefault,
                now: Timestamp::try_from(0u64).unwrap(),
    ...
    let pd = l
        .grant(
            ...
            GrantRequest {
                ...
                claim: Claim::new(Ex::Token).unwrap(),
                generation: Generation::new("g"),
                parent: Some(dev).map(|id| l.holding(id).expect("parent exists").handle()),
                lease: LeaseRequest::UseClassDefault,
                now: Timestamp::try_from(0u64).unwrap(),
            },
        )
    ...
    let w1 = l
        .grant(
            ...
            GrantRequest {
                ...
                generation: Generation::new("g"),
                parent: Some(pd).map(|id| l.holding(id).expect("parent exists").handle()),
                lease: LeaseRequest::UseClassDefault,
                now: Timestamp::try_from(0u64).unwrap(),
            },
        )
    ...
    assert_eq!(
        l.grant(
            ...
            GrantRequest {
                owner: SubjectId::new("a"),
                claim: Claim::new(Ranges::of(&[(1000, 2000)])).unwrap(),
                generation: Generation::new("g"),
                parent: Some(pd).map(|id| l.holding(id).expect("parent exists").handle()),
                lease: LeaseRequest::UseClassDefault,
                now: Timestamp::try_from(0u64).unwrap()
    ...
    let mut o = Orchestrator::new(l);
    assert!(
        matches!(o.teardown("a", 5, None), RunOutcome::Completed { failed } if failed.is_empty())
    );
    let pos = |id: HoldingId| o.world.action_order.iter().position(|x| *x == id).unwrap();
    assert!(pos(w1) < pos(pd) && pos(pd) < pos(dev));
    assert_eq!(o.ledger.live_snapshot(&SubjectId::new("b")).len(), 1);
}
