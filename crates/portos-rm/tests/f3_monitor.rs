...
fn truncation_enforces_safety_precisely() {
    ...
    assert_eq!(m.world.emitted, plan, "合法输入必须原样通过（precise 的透明性半边）");
...
fn no_truncation_strategy_passes_transaction_witness_but_edit_does() {
    let mk = |last: &str| {
        vec![a("send", "mail.corp", "a"), a("send", "mail.corp", "b"), a(last, "mail.corp", "")]
    };
    let plan_commit = mk("commit");
    let plan_abort = mk("abort");
    let content =
        |out: &[WAction]| out.iter().filter(|x| x.verb == "send").map(|x| x.payload.clone()).collect::<Vec<_>>();
    ...
    let mut m = Monitor::new(base_policy(), Mode::Strict, Ledger::new(), "fib");
    m.admit(staged_plan.clone(), H, consent("n1", 0), NOW).unwrap();
    assert_eq!(*m.run(NOW), MonState::AwaitingApproval);
    assert!(m.world.emitted.is_empty(), "悬置期与 abort 分支前缀行为一致：零发射");
    m.approve(consent("n2", 2), NOW).unwrap();
    assert!(tx_valid(&plan_commit, &m.world.emitted), "commit 分支：批准后完整按序放出");
    ...
    assert!(tx_valid(&plan_abort, &m.world.emitted), "abort 分支：零泄漏");
}
...
fn wysiwys_gate_no_consent_no_effect() {
    ...
    assert_eq!(m.admit(plan.clone(), H, bad, NOW), Err(Refusal::ConsentMismatch));
    assert!(m.world.emitted.is_empty());
    assert_eq!(m.pool_spent("n1"), 0);
    assert!(m.trace.contains(&Ev::Refused { why: Refusal::ConsentMismatch }));

    let mut m = Monitor::new(base_policy(), Mode::Strict, Ledger::new(), "fib");
    assert_eq!(m.admit(plan, H, consent("n1", 10), TTL + 1), Err(Refusal::ExpiredTtl));
    assert!(m.world.emitted.is_empty());
}
...
fn withhold_approve_emits_exactly_once_in_order() {
    ...
    assert_eq!(*m.run(NOW), MonState::AwaitingApproval);
    assert_eq!(m.world.emitted, vec![a("post", "api.example.com", "y")], "压制期：staged 件零可见");
    assert_eq!(
        m.trace.iter().filter(|e| matches!(e, Ev::Suppressed { .. })).count(),
        2
    );
    ...
    assert_eq!(m.approve(consent("n2", 1), NOW), Err(Refusal::ApprovalBudgetShort));
    ...
    let count_x =
        m.world.emitted.iter().filter(|e| e.payload == "x").count();
    assert_eq!(count_x, 1, "恰好一次");
    assert_eq!(m.approve(consent("n4", 2), NOW), Err(Refusal::WrongState), "无重复插入窗口");
...
fn ttl_bounds_feigning_acceptance_expiry_compensates() {
    ...
    l.set_capacity("reservation", "email-42", Frag::Ex(Ex::Token));

    let mut m = Monitor::new(base_policy(), Mode::Strict, l, "fib");
    m.admit(vec![a("send", "mail.corp", "x")], H, consent("n1", 0), NOW).unwrap();
    // reserve→同意→commit 的 reserve 步：段内押下一笔可补偿持有。
    m.stage_acquire("reservation", "email-42", "g", Frag::Ex(Ex::Token), NOW).unwrap();
    assert_eq!(*m.run(NOW), MonState::AwaitingApproval);

    // 未到期不得废（废也要有据）。
    assert_eq!(m.expire(NOW), Err(Refusal::WrongState));
    // 过了 ttl：批准窗口已死——同意已过期，段只能废。
    assert_eq!(m.approve(consent("n2", 2), TTL + 1), Err(Refusal::ExpiredTtl));
    ...
    assert_eq!(m.orch.ledger.live_snapshot(&m.seg_subject()).len(), 0);
    assert!(m.trace.contains(&Ev::SegmentAborted { expired: true }));

    // 迟到的批准无处落地。
    assert_eq!(m.approve(consent("n3", 2), TTL + 2), Err(Refusal::WrongState));
}
...
fn paused_segment_is_bounded_by_original_ttl() {
    ...
    let plan: Vec<WAction> = (1..=3).map(|i| a("post", "api.example.com", &i.to_string())).collect();
    ...
    assert_eq!(m.resume_with(fresh, TTL + 1).unwrap_err(), Refusal::ExpiredTtl);
    assert!(m.trace.contains(&Ev::Refused { why: Refusal::ExpiredTtl }), "拒绝可听见");
    ...
    assert_eq!(m.orch.ledger.live_snapshot(&m.seg_subject()).len(), 0);
    assert!(!m.is_suspended());
    assert_eq!(m.resume_with(consent("n3", 5), TTL + 2), Err(Refusal::WrongState), "废段无处续跑");
}
...
fn attenuate_only_by_declared_equivalence() {
    ...
    let mut m = Monitor::new(base_policy(), Mode::Strict, Ledger::new(), "fib");
    m.admit(vec![a("read_full", "quiet.internal", "doc")], H, consent("n1", 10), NOW).unwrap();
    assert_eq!(*m.run(NOW), MonState::Done(MonOutcome::Completed));
    assert_eq!(m.world.emitted, vec![a("read_preview", "quiet.internal", "doc")]);
    assert!(m
        .trace
        .contains(&Ev::Attenuated { from: "read_full".into(), to: "read_preview".into() }));

    // 无声明：同一越界目标、未声明动词 ⇒ fail-stop，世界零效应。
    let mut m = Monitor::new(base_policy(), Mode::Strict, Ledger::new(), "fib");
    m.admit(vec![a("scrape", "quiet.internal", "doc")], H, consent("n1", 10), NOW).unwrap();
    ...
    let mut m = Monitor::new(base_policy(), Mode::Strict, Ledger::new(), "fib");
    m.admit(vec![a("read_full", "evil.example", "doc")], H, consent("n1", 10), NOW).unwrap();
    assert_eq!(*m.run(NOW), MonState::Done(MonOutcome::FailStop { at: 0 }));
...
fn insertion_is_still_an_emission_sink_holds_through_approval() {
    ...
    let mut m = Monitor::new(base_policy(), Mode::Strict, Ledger::new(), "fib");
    let plan = vec![a("send", "evil.example", "leak"), a("send", "mail.corp", "ok")];
    m.admit(plan, H, consent("n1", 0), NOW).unwrap();
    assert_eq!(*m.run(NOW), MonState::Done(MonOutcome::FailStop { at: 0 }));
    assert!(m.world.emitted.is_empty(), "越界 staged 件绝不可经批准漏入世界");
    assert_eq!(m.approve(consent("n2", 9), NOW), Err(Refusal::WrongState), "无缓冲可批");

    // 隔离目标的 staged：进替身（隔离优先于扣发——替身不是真实边界，无需同意）。
    let mut m = Monitor::new(base_policy(), Mode::Strict, Ledger::new(), "fib");
    m.admit(vec![a("send", "sketchy.site", "s")], H, consent("n1", 0), NOW).unwrap();
    assert_eq!(*m.run(NOW), MonState::Done(MonOutcome::Completed));
    assert!(m.world.emitted.is_empty());
...
fn confine_redirects_to_stand_in_zero_real_effect() {
    let mut m = Monitor::new(base_policy(), Mode::Strict, Ledger::new(), "fib");
    let plan = vec![a("post", "sketchy.site", "s"), a("post", "api.example.com", "r")];
    m.admit(plan, H, consent("n1", 10), NOW).unwrap();
    assert_eq!(*m.run(NOW), MonState::Done(MonOutcome::Completed));
    assert_eq!(m.world.emitted, vec![a("post", "api.example.com", "r")], "真实通道零替身件");
    assert_eq!(m.world.standin, vec![a("post", "sketchy.site", "s")], "替身通道完整承接");
    assert!(m.trace.contains(&Ev::Confined { verb: "post".into(), target: "sketchy.site".into() }));
}
...
fn escalate_pauses_and_resumes_exactly_with_fresh_consent() {
    ...
    let plan: Vec<WAction> =
        (1..=4).map(|i| a("post", "api.example.com", &i.to_string())).collect();
    ...
    assert_eq!(m.resume_with(consent("n1", 2), NOW).unwrap_err(), Refusal::StaleNonce);
    let mut wrong = consent("n2", 2);
    wrong.plan_hash = "blake3:other".into();
    assert_eq!(m.resume_with(wrong, NOW).unwrap_err(), Refusal::ConsentMismatch);
    assert_eq!(m.world.emitted.len(), 2, "被拒的增量同意不得放出任何动作");

    // 增量同意可以再次不够——escalate 循环必须可组合（真实节奏就是挤牙膏式增批）。
    assert_eq!(*m.resume_with(consent("n2", 1), NOW).unwrap(), MonState::Paused);
    assert_eq!(m.world.emitted, plan[..3].to_vec(), "第二次停下：又一段预算内前缀");
    assert_eq!(*m.resume_with(consent("n3", 1), NOW).unwrap(), MonState::Done(MonOutcome::Completed));
    assert_eq!(m.world.emitted, plan, "续跑：前缀不重放、暂停件不跳过、按序完成");
    for i in 1..=4 {
        let n = m.world.emitted.iter().filter(|e| e.payload == i.to_string()).count();
        assert_eq!(n, 1, "动作 {i} 恰好一次");
    }
    assert_eq!(m.pool_spent("n1") + m.pool_spent("n2") + m.pool_spent("n3"), 4, "三池合计＝总发射");
}
...
fn budget_is_rows_not_decrement_gate_is_issuer_gate() {
    let mut m = Monitor::new(base_policy(), Mode::Strict, Ledger::new(), "fib");
    let plan: Vec<WAction> =
        (1..=5).map(|i| a("post", "api.example.com", &i.to_string())).collect();
    ...
    assert_eq!(
        m.orch.ledger.live().filter(|h| h.class_id == "budget").count(),
        3,
        "一笔花费一行，行为真相"
    );
...
fn strict_failstop_delivers_prefix_and_rolls_back_segment_holdings() {
    ...
    l.set_capacity("scratch", "s1", Frag::Ex(Ex::Token));
    l.set_capacity("scratch", "s2", Frag::Ex(Ex::Token));
    ...
    m.admit(plan.clone(), H, consent("n1", 10), NOW).unwrap();
    m.stage_acquire("scratch", "s1", "g", Frag::Ex(Ex::Token), NOW).unwrap();
    m.stage_acquire("scratch", "s2", "g", Frag::Ex(Ex::Token), NOW).unwrap();

    assert_eq!(*m.run(NOW), MonState::Done(MonOutcome::FailStop { at: 2 }));
    assert_eq!(m.world.emitted, plan[..2].to_vec(), "前缀交付");

    // [SEG-TX] 回滚由 monitor 在终态自动完成——不靠上层记得调 rollback_segment。
    assert_eq!(m.orch.ledger.live_snapshot(&m.seg_subject()).len(), 0, "段持有清零");
    ...
    assert_eq!(m.trace.iter().filter(|e| matches!(e, Ev::SegmentRolledBack { .. })).count(), 1, "重复回滚幂等");
}
...
fn segment_is_a_transaction_commit_promotes_abort_rolls_back() {
    ...
    m.admit(vec![a("post", "api.example.com", "1"), a("post", "evil.example", "2")], H, consent("n1", 10), NOW).unwrap();
    let h1 = m.stage_acquire("scratch", "s1", "g", Frag::Ex(Ex::Token), NOW).unwrap();
    let h2 = m.stage_acquire("scratch", "s2", "g", Frag::Ex(Ex::Token), NOW).unwrap();
    assert_eq!(m.promote(h2, "wrong-gen"), Err(PromoteError::Ledger(LedgerError::StaleGeneration)));
    m.promote(h1, "g").unwrap();
    assert!(m.trace.contains(&Ev::Promoted { holding: h1 }));
    assert_eq!(*m.run(NOW), MonState::Done(MonOutcome::FailStop { at: 1 }));
    assert_eq!(m.orch.ledger.live_snapshot(&m.seg_subject()).len(), 0, "段内清零");
    assert_eq!(m.orch.ledger.live_snapshot("fib").iter().map(|it| it.instance.clone()).collect::<Vec<_>>(), vec!["s1"], "提前 commit 的幸存");
    let (released, _) = m.orch.world.effects_fingerprint();
    assert_eq!(released.iter().map(|(_, i, _)| i.clone()).collect::<Vec<_>>(), vec!["s2"], "只有未 promote 的被释放");
    assert!(m.trace.contains(&Ev::SegmentRolledBack { holdings: 1 }));
    assert_eq!(m.promote(h2, "g"), Err(PromoteError::SegmentClosed), "终态后无可提前");

    // commit 分支：走完即 commit，段内剩余持有全部转授给 fiber，世界零释放。
    let mut m = Monitor::new(base_policy(), Mode::Strict, scratch(), "fib");
    m.admit(vec![a("post", "api.example.com", "1")], H, consent("n1", 10), NOW).unwrap();
    m.stage_acquire("scratch", "s1", "g", Frag::Ex(Ex::Token), NOW).unwrap();
    m.stage_acquire("scratch", "s2", "g", Frag::Ex(Ex::Token), NOW).unwrap();
    assert_eq!(*m.run(NOW), MonState::Done(MonOutcome::Completed));
    assert_eq!(m.orch.ledger.live_snapshot(&m.seg_subject()).len(), 0);
    assert_eq!(m.orch.ledger.live_snapshot("fib").len(), 2, "commit：全部归 fiber");
    assert!(m.orch.world.effects_fingerprint().0.is_empty(), "commit 不动世界");
    ...
    m.admit(vec![a("post", "api.example.com", "1"), a("post", "api.example.com", "2")], H, consent("n1", 1), NOW).unwrap();
    m.stage_acquire("scratch", "s1", "g", Frag::Ex(Ex::Token), NOW).unwrap();
    assert_eq!(*m.run(NOW), MonState::Done(MonOutcome::Truncated { dropped: 1 }));
    assert_eq!(m.world.emitted.len(), 1, "前缀站着不动");
    assert_eq!(m.orch.ledger.live_snapshot(&m.seg_subject()).len(), 0);
    assert_eq!(m.orch.world.effects_fingerprint().0.len(), 1, "截断也回滚段内持有");
}
...
fn truncate_never_silent() {
    ...
    assert_eq!(*t.run(NOW), MonState::Done(MonOutcome::Truncated { dropped: 3 }));
    assert_eq!(t.world.emitted, plan[..2].to_vec());
    assert!(t.trace.contains(&Ev::Truncation { dropped: 3 }), "截断事件必须可听见");
    ...
    assert_eq!(*s.run(NOW), MonState::Done(MonOutcome::FailStop { at: 2 }), "同界不同结局");
