    ...
    for commit in [false, true] {
        ...
        for name in ["member", "unrelated"] {
            pools.push(
                ledger
                    .create_pool(
                        &ledger.registered_class::<Ex>(&class).unwrap(),
                        InstanceId::new(name),
                        Capacity::new(Ex::Token).unwrap(),
                    )
                    .unwrap(),
        ...
        let member = monitor
            ...
            .unwrap();
        assert_eq!(
            monitor.promote(&outsider),
            Err(PromoteError::Ledger(LedgerError::ForgedHandle))
        );
        monitor.run(NOW);
        ...
        if commit {
            assert_eq!(member.subject, SubjectId::new("fib"));
        } else {
            assert!(member.released_at().is_some());
        }
        assert!(
            monitor
                .stage_acquire(
