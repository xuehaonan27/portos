...
fn comp(with: &str, amortizable: bool) -> VerbEntry {
    VerbEntry::emitting(EmitGrade::Compensable { compensate_with: with.into() }, amortizable)
}
...
fn browser_verb_table() -> VerbTable {
    ...
    for c in ["enclosure", "proc", "page", "tcp-port", "workspace", "socket"] {
        t.declare_class(c, RevertGrade::Inverse).unwrap();
    }
    t.declare_class("reservation", RevertGrade::Compensable).unwrap();
    ...
    t.register("page", "submit", ext(false).degrades_to("submit_draft")).unwrap();
    t.register("page", "submit_draft", comp("discard_draft", true)).unwrap();
    ...
    t.register("tcp-port", "probe", VerbEntry::repeatable()).unwrap();
    ...
    t.register("socket", "recv", VerbEntry::consuming(ConsumeGrade::External)).unwrap();
    ...
    t.register("doc", "read_full", VerbEntry::repeatable().degrades_to("read_preview")).unwrap();
    t.register("doc", "read_preview", VerbEntry::repeatable()).unwrap();
    // api：可补偿发射＋其补偿动词。
    t.register("api", "post", comp("post_cancel", true)).unwrap();
...
fn coherence_lattice_exhaustive_over_kind_and_flags() {
    let kinds: Vec<(&str, Kind)> = vec![
        ...
        ("Consuming/Held", Kind::Consuming { world: ConsumeGrade::Held }),
        ("Consuming/Comp", Kind::Consuming { world: ConsumeGrade::Compensable { compensate_with: "c".into() } }),
        ("Consuming/Ext", Kind::Consuming { world: ConsumeGrade::External }),
        ("Emitting/Comp/amort", Kind::Emitting { world: EmitGrade::Compensable { compensate_with: "c".into() }, amortizable: true }),
        ("Emitting/Comp/hard", Kind::Emitting { world: EmitGrade::Compensable { compensate_with: "c".into() }, amortizable: false }),
        ("Emitting/Ext/amort", Kind::Emitting { world: EmitGrade::External, amortizable: true }),
        ("Emitting/Ext/hard", Kind::Emitting { world: EmitGrade::External, amortizable: false }),
    ];
    ...
    for (name, kind) in &kinds {
        for idem in [false, true] {
            for comm in [false, true] {
                let e = VerbEntry { kind: kind.clone(), idempotent: idem, commutes: comm, degrade: None };
                let got = e.check_coherent().is_ok();
                assert_eq!(got, expected_ok(kind, idem, comm), "格 {name} idem={idem} comm={comm} 判定与规则表不符");
                cells += 1;
                accepted += got as u32;
            }
        }
    ...
    assert!(!shared.bears_budget() && shared.blind_replay_safe() && !shared.commutes);
    assert!(VerbEntry::repeatable().commutes);
    assert!(VerbEntry::repeatable().with_flags(false, true).check_coherent().is_err(), "可重复仍须幂等");
}
...
fn d1_same_verb_two_handlers_projections_differ() {
    ...
    t.register("cas", "read", VerbEntry::repeatable()).unwrap();
    t.register("queue", "read", VerbEntry::consuming(ConsumeGrade::External)).unwrap();
    assert!(!t.lookup("cas", "read").unwrap().bears_budget());
    assert!(t.lookup("queue", "read").unwrap().bears_budget(), "预算地位随手柄翻转");
    assert_eq!(t.lookup("cas", "write"), Err(VerbError::Unknown), "未注册 (类,动词) 是硬错");

    // B6 墓碑：同名 send 的扣发地位随 handler 而异——按类投影才保得住 D1。
    assert!(t.derive_handler_policy("mail").withhold.contains("send"));
    assert!(!t.derive_handler_policy("bus").withhold.contains("send"));
    assert!(!t.derive_handler_policy("socket").withhold.contains("send"));
    for h in ["mail", "bus", "socket"] {
        assert!(t.derive_handler_policy(h).budget.contains("send"), "{h}.send 都进预算");
    }
    assert_eq!(t.derive_handler_policy("bus").compensations.get("send").map(String::as_str), Some("retract"));
}
...
fn consuming_read_bears_budget_refines_read_write_binary() {
    ...
    let recv = t.lookup("socket", "recv").unwrap();
    let snap = t.lookup("page", "snapshot").unwrap();
    assert!(!matches!(recv.kind, Kind::Emitting { .. }) && !matches!(snap.kind, Kind::Emitting { .. }));
    assert!(recv.bears_budget(), "队列 pop 是真实状态变更，必须进预算");
    assert!(!snap.bears_budget(), "不可变源读不占预算，只记标签");
    assert!(snap.blind_replay_safe() && !recv.blind_replay_safe(), "可重复读盲重放安全，消耗读不安全");
}
...
fn holding_rho_is_per_class_action_grade_is_per_verb() {
    ...
    assert_eq!(t.derive_holding_grade("page"), Some(RevertGrade::Inverse));
    // ……而它的发射动词各有自己的世界档，与持有档无关。
    assert!(t.lookup("page", "click").unwrap().staged_shape(), "click 是外部发射");
    assert!(matches!(t.lookup("page", "submit_draft").unwrap().kind, Kind::Emitting { world: EmitGrade::Compensable { .. }, .. }));
    assert!(matches!(t.lookup("page", "attach").unwrap().kind, Kind::Consuming { world: ConsumeGrade::Held }));
    // socket：连接是持有、recv 外部消耗、send 外部发射——一类三性格。
    assert_eq!(t.derive_holding_grade("socket"), Some(RevertGrade::Inverse));
    assert!(t.lookup("socket", "send").unwrap().staged_shape());
    assert!(matches!(t.lookup("socket", "recv").unwrap().kind, Kind::Consuming { world: ConsumeGrade::External }));
    // 无持有的 handler 没有 ρ（None 不是缺省档，是"此类不产生持有"）。
    assert_eq!(t.derive_holding_grade("mail"), None);
}
...
fn withhold_iff_non_amortizable_staged_shape_iff_external() {
    let t = browser_verb_table();
    let click = t.lookup("page", "click").unwrap();
    assert!(click.staged_shape() && !click.withhold(), "click：两阶段由准入满足，不逐次扣发");
    let submit = t.lookup("page", "submit").unwrap();
    assert!(submit.staged_shape() && submit.withhold(), "submit：硬清单，逐次同意");
    let draft = t.lookup("page", "submit_draft").unwrap();
    assert!(!draft.staged_shape() && !draft.withhold(), "可补偿发射：既非两阶段也不扣发");
    let post = t.lookup("api", "post").unwrap();
    assert!(!post.staged_shape() && !post.withhold());
    // 可补偿但在硬清单上（删除→可从回收站恢复）：不是两阶段形状，但仍逐次同意。
    let mut s = VerbTable::new();
    s.register("fs", "delete", comp("restore", false)).unwrap();
    s.register("fs", "restore", ext(true)).unwrap();
    s.check_all().unwrap();
    let del = s.lookup("fs", "delete").unwrap();
    assert!(!del.staged_shape() && del.withhold(), "硬清单优先于可补偿性");
}
...
fn degrade_declared_and_only_narrows() {
    ...
    assert_eq!(t.derive_handler_policy("doc").degrade.get("read_full").map(String::as_str), Some("read_preview"));
    assert_eq!(t.derive_handler_policy("page").degrade.get("submit").map(String::as_str), Some("submit_draft"));

    // 升档伪装成降档：submit_draft（可补偿、可摊销）→ submit（外部、硬清单）——拒。
    let mut u = VerbTable::new();
    u.register("page", "submit", ext(false)).unwrap();
    u.register("page", "submit_draft", comp("discard", true).degrades_to("submit")).unwrap();
    u.register("page", "discard", ext(true)).unwrap();
    assert!(matches!(u.check_all(), Err(VerbError::Incoherent(_))), "降档不得升严重度");

    // 目标未注册／跨类：拒（等价必须指向同 handler 下的已声明动词）。
    let mut v = VerbTable::new();
    v.register("doc", "read_full", VerbEntry::repeatable().degrades_to("read_preview")).unwrap();
    assert!(matches!(v.check_all(), Err(VerbError::Incoherent(_))));
    v.register("other", "read_preview", VerbEntry::repeatable()).unwrap();
    assert!(matches!(v.check_all(), Err(VerbError::Incoherent(_))), "跨类目标不算");
}
...
fn held_requires_declared_class_rho_and_rho_is_immutable() {
    let mut t = VerbTable::new();
    assert_eq!(t.register("port", "bind", held()), Err(VerbError::ClassNotDeclared));
    t.declare_class("port", RevertGrade::Inverse).unwrap();
    t.register("port", "bind", held()).unwrap();
    assert_eq!(t.declare_class("port", RevertGrade::External), Err(VerbError::ClassAlreadyDeclared));
    assert_eq!(t.derive_holding_grade("port"), Some(RevertGrade::Inverse));
    // 补偿动词自己又要补偿——链不闭合，拒。
    let mut c = VerbTable::new();
...
fn truth_table_is_shared_source_for_f2_teardown_and_f3_monitor() {
    ...
    for (class, expect) in [
        ...
        assert_eq!(t.derive_holding_grade(class), Some(expect), "F2 档投影与 browser_ledger 手写值不符：{class}");
    }
    ...
    for class in ["enclosure", "reservation"] {
        l.register_class(ClassDecl {
            ...
            lease_secs: Some(300),
            revert_grade: t.derive_holding_grade(class).unwrap(),
        });
    }
    l.set_capacity("enclosure", "e", Frag::Ex(Ex::Token));
    l.set_capacity("reservation", "r", Frag::Ex(Ex::Token));
    let e = l.grant("s", "enclosure", "e", Frag::Ex(Ex::Token), "g", None, 0).unwrap();
    l.grant("s", "reservation", "r", Frag::Ex(Ex::Token), "g", Some(e), 0).unwrap();
    let mut o = Orchestrator::new(l);
    assert!(matches!(o.teardown("s", 0, None), RunOutcome::Completed { failed } if failed.is_empty()));
    let (_, comp) = o.world.effects_fingerprint();
    assert_eq!(comp.len(), 1, "表投影的 Compensable 类档驱动 F2 补偿恰好一次");

    // ---- 投影到 F3：page handler 的策略 → Policy，跑一批自治动作 ----
    let hp = t.derive_handler_policy("page");
    assert_eq!(hp.withhold, ["submit".to_string()].into_iter().collect(), "只有硬清单进扣发集");
    assert!(hp.budget.contains("click") && hp.budget.contains("submit") && !hp.budget.contains("snapshot"));
    ...
    let consent = |n: &str, b: u64| Consent { plan_hash: "blake3:p".into(), budget: b, nonce: n.into(), ttl_expires_at: 100 };
    m.admit(plan, "blake3:p", consent("n1", 2), 10).unwrap();
    assert_eq!(*m.run(10), MonState::AwaitingApproval, "硬清单 submit 扣发待批");
    assert_eq!(m.world.emitted.len(), 2, "可摊销的 click/type 在一次同意下直发——一次同意换一批");
    m.approve(consent("n2", 1), 10).unwrap();
    assert_eq!(m.world.emitted.len(), 3, "批准后 submit 放出");

    // mail handler：同一张表投影出另一台监督器的扣发集（D1）。
    assert_eq!(t.derive_handler_policy("mail").withhold, ["send".to_string()].into_iter().collect());
}
